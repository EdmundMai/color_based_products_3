...
Then(/^there shouldn't be a form to create an account$/) do
  expect(page).not_to have_field("payment_info[email]")
  expect(page).not_to have_field("payment_info[create_an_account]")
...
Then(/^the review button should be enabled$/) do
  expect(find_button("Review")).not_to be_nil
end
...
Then(/^my user should be updated$/) do
  order = Order.last
  user = order.user
  expect(user.email).to eq("abc123@nettheory.com")
...
Then(/^my subtotal should be discounted by a flat amount$/) do
  ...
  cart = Cart.last
  flat_coupon = FlatCoupon.last
  expect(order.subtotal).to eq(cart.total - Money.new(flat_coupon.value*100))
...
Then(/^my subtotal should be discounted by a percentage amount$/) do
  ...
  cart = Cart.last
  percentage_coupon = PercentageCoupon.last
  expect(order.subtotal).to eq(cart.total * (100 - percentage_coupon.value) / 100)
